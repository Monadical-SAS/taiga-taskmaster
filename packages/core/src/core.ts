...
import {
  bang,
  castNonNegativeInteger, type NonEmptyString,
  type NonNegativeInteger,
  PrdText, type PrdTextHash,
  SINGLETON_PROJECT_ID, type TaskId
...
namespace TasksMachine {
  ...
  type Artifact = {
    ...
    tasks: Tasks & {/*status: completed... TODO*/}
  }
  ...
  export type State = {
    ...
    tasks: Tasks, // those are not ALL the tasks; we have "done" tasks but not merged yet; we can have a "done" non-merged task edited - that's a command to redp/cleanup
  ...
  const uniqAppend = (tx1: Tasks, tx2: Tasks) => {
    ...
    if (HashMap.size(r) !== expected) throw new Error(`panic! uniqAppend argument keys intersect: ${pipe(HashMap.keySet(tx2), HashSet.intersection(HashMap.keySet(tx1)))}`)
    return r;
  }

  const anyKeyIntersection = (tx1: Tasks, tx2: Tasks) => {
    return pipe(HashMap.keySet(tx1), HashSet.intersection(HashMap.keySet(tx2)), HashSet.size) > 0;
  }

  const allTasks = (s: State) => s.artifacts.reduce((a, b) => uniqAppend(a, b.tasks), s.tasks);

  // simplest operation, add more tasks
  export const appendTasks = (tasks: Tasks) => (s: State): State => {
    if (anyKeyIntersection(allTasks(s), tasks)) {
      throw new Error(`panic! tasks to append will create duplicates: ${pipe(
        allTasks(s),
        HashMap.keySet,
  ...
  export const commitArtifact = (aid: ArtifactId) => (s: State): State => {
    if (s.artifacts.length === 0) {
      throw new Error(`panic! artifact ${aid} not found in pending artifacts, moreover artifacts are empty`);
    }
    ...
    if (HashMap.size(tasks) === 0) {
      throw new Error(`panic! artifact tasks size invariant violation: an artifact always has at least 1 task`);
    }
    // TODO validate every is complete additionally, otherwise we risk forever loops
    // if (HashMap.values(tasks))
    if (artifact.id !== aid) throw new Error(`panic! artifact to commit ${aid} isn't next in line; expected ${artifact.id}`);
    return {
      ...s,
...
namespace AgentExecution {
  ...
  export const summarize = async (s: AgentExecutionState & {step: "running"}): Promise<AgentExecutionState> => {
