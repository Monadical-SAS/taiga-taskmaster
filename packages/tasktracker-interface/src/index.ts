import {
  ...
  ProjectId,
  TaskFileContent,
  type UniqTaskFileContentList, bang
...
import {
  ...
  decodeTaskIdFromTag,
  encodeProjectIdToTag, encodeTaskIdToTag,
  ProjectIdTag,
  TaskIdTag
} from './tags.js';
import { ProjectId as TaigaProjectId, TaskDetail, type TasksService } from '@taiga-task-master/taiga-api-interface';
...
export const syncTasks: SyncTasksF = (di) => async (tasks, projectId) => {
  ...
  const currentIds = await getTasks(di.getTasks)(idsToUpdateOrCreate, projectId);
...
export const ValidTaskTransformation = Schema.transformOrFail(
  ...
  Schema.typeSchema(ValidTask),
  {
    strict: true,
    decode: (taigaTask) => {
      const [projectIdTags, taskIdTags] = splitByProjectIdAndTaskIdTags(taigaTask.tags)
      if (projectIdTags.length !== 1) {
        return ParseResult.fail(new Unexpected(`projectIdTags length in taiga task ${taigaTask.id} is ${projectIdTags.length}`));
      }
      if (taskIdTags.length !== 1) {
        return ParseResult.fail(new Unexpected(`taskIdTags length in taiga task ${taigaTask.id} is ${taskIdTags.length}`));
      }
      ...
      return ParseResult.succeed(
        {
          ...
          tags: [Tuple.make(encodeProjectIdToTag(validatedTask.masterProjectId), null), Tuple.make(encodeTaskIdToTag(validatedTask.masterId), null)]
        }
...
// TODO add tests
export const filterTasks = (expected: Set<TaskId>, allTasks: readonly TaskDetail[], projectId: ProjectId): {
  ...
  const decode = (t: TaskDetail, _: number) => Schema.decodeEither(ValidTaskTransformation)(t)
  const filteredTasks = allTasks.map(decode).reduce((acc: {
    valid: Map<TaskId, ValidTask>,
    dupes: Set<TaskId>,
    unrelatedProject: Set<TaskId>,
    extra: Set<TaskId>,
    ...
    if (isLeft(e)) {
      console.info(`a project manager task format was wrong, ignoring: ${ParseResult.TreeFormatter.formatErrorSync(e.left)}`);
      return {
        ...acc,
    ...
    if (!expected.has(t.masterId)) {
      console.info(`a master project ${projectId} has an expected task with master id ${t.masterId}: ${t.id}`)
      return {
        ...acc,
    ...
    if (acc.valid.has(t.masterId)) {
      console.info(`a task of master project ${projectId} ${t.masterId}: ${t.id} is duped`)
  ...
  const notFound = pipe(HashSet.make(...expected.values()), HashSet.difference(found));
  return {
    ...filteredTasks,
...
const getTasks = (deps: SyncTasksDeps['getTasks']) => async (
  ids: Set<TaskId>,
  projectId: ProjectId,
): Promise<Set<TaskId>> => {
...
function splitByProjectIdAndTaskIdTags(tags: TaskDetail['tags']): [ProjectIdTag[], TaskIdTag[]]{
  return pipe(
    tags,
    filterMap(([tt]): Option.Option<{
      tag: ProjectIdTag,
      type: 'projectId'
    } | {
      tag: TaskIdTag,
