...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `ℹ️  Tasks API test: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
    ...
    try {
      ...
      console.log(
        `✅ User Stories API works! Found ${userStories.length} user stories`
      );
    } catch (error) {
      console.log(
        `ℹ️  User Stories API test: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
    ...
    console.log(
      "🔄 Credential storage and login retry on refresh failure is now enabled"
    );
  } catch (error) {
    console.error(
