...
const createFakeHttpClient = (
  config: FakeHttpClientConfig
): HttpClient & {
  ...
  return {
    get: async (
      ...
      if (
        path.includes("/api/v1/tasks") ||
        path.includes("/api/v1/userstories")
      ) {
        callCounts.api++;
...
const main = async (): Promise<void> => {
  ...
  // ============================================================================
  results.push(
    await runTest("Token Refresh Failure → Login Retry Success", async () => {
      ...
      const { api, refreshWithStoredToken, getAuthToken } = createAuthService(
        fakeClient,
        credentials
      );
  ...
  // ============================================================================
  results.push(
    await runTest("Login Failure Scenario", async () => {
      const fakeClient = createFakeHttpClient({
      ...
      try {
        ...
        if (!(error instanceof Error && error.message.includes("HTTP 401"))) {
          throw new Error(
            `Expected HTTP 401 error, got: ${error instanceof Error ? error.message : error}`
          );
        }
      }
  ...
  // ============================================================================
  results.push(
    await runTest("Network Failure Scenario", async () => {
      const fakeClient = createFakeHttpClient({
      ...
      const { api, refreshWithStoredToken } = createAuthService(
        fakeClient,
        credentials
      );
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            `Expected network error, got: ${error instanceof Error ? error.message : error}`
          );
        }
      }
  ...
  // ============================================================================
  results.push(
    await runTest("No Stored Credentials Scenario", async () => {
      const fakeClient = createFakeHttpClient({
        authEndpointBehavior: "success",
        ...
        if (
          !(
            ...
            error.message.includes(
              "No refresh token or stored credentials available"
            )
          )
        ) {
          throw new Error(
            `Expected 'no stored credentials' error, got: ${error instanceof Error ? error.message : error}`
          );
        }
      }
    })
  ...
  // ============================================================================
  results.push(
    await runTest("Full Flow with Multiple Retries", async () => {
      const fakeClient = createFakeHttpClient({
        authEndpointBehavior: "success",
        refreshEndpointBehavior: "success",
        apiEndpointBehavior: "auth-required",
